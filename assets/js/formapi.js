...
jQuery.validator.addMethod('validUserName', function (value) 
{
 ...
 jQuery.validator.addMethod('validEmailofPatient',function(value)
  {
...
function TrimText(el) {
  ...
  //   let dt = "C:\fakepath\jen.jpg";
  //   let data = dt.replace(/(^\s*)|(\s*$)/gi, "").
  // replace(/[ ]{2,}/gi, " ").
  // replace(/\n +/, "\n");
...
$("#loginform").validate({
    ...
    messages : {
      ...
      minlength:"Password must be of 8 characters| يجب أن تتكون كلمة المرور من 8 أحرف ",
      },
   ...
   if(phy_email!="" &&  phy_password !=="" && phy_password.length >=8){
    $.ajax({
      ...
      $('#hidepp').html("Email or password is wrong").removeClass('hidep');
        ...
        else if(res.message == "Please wait for the approval email to the admin, after you can log in successfully."){
          //  $('#message').html(res.message).addClass('alert alert-danger');
           // $('#hidepp').html(res.message).removeClass('hidep');
            $('#hidepp').html('Please wait for admin approval').removeClass('hidep');
        }
...
//    $('#pharma_signup').submit(function (event) { alert("hello from formapi.js")
//     event.preventDefault();
...
/*Pharmacist Signup API Implementation*/
  ...
  /*formvalidation for signup form*/
    var pham_name =$('pham_name').val().upperFirst();
    var pham_first_name = $('#pham_first_name').val();
    var pham_last_name = $('#pham_last_name').val();
    var email = $('#pham_email').val();
    var pham_regn =$('#pham_regn').val();
    ...
    if(pham_name!="" && pham_first_name!="" && pham_last_name !=="" && email!="" && pham_regn!="" && password!="" ){
    $.ajax({
      type: "POST",
      url: api_url+"phamsistRegister",
      data: new FormData(this),
     ...
     //    alert('done');
      ...
      // return false;
        ...
        console.log("Inside else of assets formapi.js")
        $('#message').html(res.message,"aslkdls").addClass('alert alert-danger');
           // $(':input').focus(function(){
           //  $('#emdhide').hide()
           // })
